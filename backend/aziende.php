...
class Aziende extends BaseModel
{
    private $table = 'aziende';

    public function find($id)
    {
        $stmt = $this->conn->prepare("SELECT aziende.*, comuni.comune AS comune, province.provincia AS provincia, regioni.regione AS regione
                                      FROM aziende
                                      LEFT JOIN comuni ON aziende.id_comune = comuni.id
                                      LEFT JOIN province ON aziende.id_provincia = province.id
                                      LEFT JOIN regioni ON aziende.id_regione = regioni.id
                                      WHERE aziende.id = ?");
    ...
    public function all()
    {
        $query = "SELECT aziende.*, comuni.comune AS comune, province.provincia AS provincia, regioni.regione AS regione
                  FROM aziende
                  LEFT JOIN comuni ON aziende.id_comune = comuni.id
                  LEFT JOIN province ON aziende.id_provincia = province.id
                  LEFT JOIN regioni ON aziende.id_regione = regioni.id";
    ...
    public function update($id, $data)
    {
        ...
        if (!$stmt->execute()) {
            throw new Exception("Errore nell'aggiornamento dell'azienda: " . $this->conn->error);
        }
    ...
    public function getWithFilters($filters = [])
    {
        $query = "SELECT 
                ...
                FROM aziende
                JOIN comuni ON aziende.id_comune = comuni.id
                JOIN province ON aziende.id_provincia = province.id
                JOIN regioni ON aziende.id_regione = regioni.id
                WHERE 1";

        if (!empty($filters['cliente'])) {
            $searchTerm = $this->conn->real_escape_string($filters['cliente']);
            $query .= " AND (CONCAT(aziende.nome, ' ', aziende.cognome) LIKE '%$searchTerm%' 
